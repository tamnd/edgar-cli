...
describes what users get. Pick a format with `-o`, or let edgar choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
edgar <command> -o table   # aligned columns for reading
edgar <command> -o jsonl   # one JSON object per line, for piping
edgar <command> -o json    # a single JSON array
edgar <command> -o csv     # spreadsheet friendly
edgar <command> -o tsv     # tab-separated
edgar <command> -o url     # just the URL column
edgar <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
edgar <command> --fields id,title,url
```
...
```bash
edgar <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
edgar <command>            # a table, because this is a terminal
edgar <command> | wc -l    # JSONL, because this is a pipe
```
